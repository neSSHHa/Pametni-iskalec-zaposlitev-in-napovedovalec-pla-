...
public class CareerJetGetNewJobsAU {
    ...
    private static void saveToFile(JSONArray data) throws Exception {
        ...
        Path file = folder.resolve("newJobsAU.json");

        Files.writeString(file, data.toString(2));

        System.out.println("Saved " + data.length() + " unique recent jobs to data/newJobsAU.json");
    }
}
